...
# 1. READ ORIGINAL FILES FROM BASE COMMIT
...
# 2. DEFINE THE UNIFIED FULL-WIDTH TOP NAVIGATION CSS
SHARED_NAV_CSS = """
<style id="lula-nav-core">
/* === LULASYNC FULL-WIDTH TOP NAVIGATION === */
nav.navbar, .navbar {
  position: sticky !important;
  top: 0 !important;
  z-index: 999999 !important;
  background: rgba(15, 16, 17, 0.94) !important;
...
# 3. DEFINE THE UNIFIED NAVBAR HTML
def get_unified_nav_html(prefix=""):
    return f"""
      ...
      <div class="navbar_content">
        <div animation="navbar-content" class="padding-global is-tiny">
          <div class="navbar_component">
            <!-- LEFT: LOGO & PERSONA SELECTOR RIGHT NEXT TO IT -->
                ...
                <div class="lula-nav-link-item">
                  ...
                  <div class="lula-mega" id="mega-whatwedo" onclick="event.stopPropagation()">
                    ...
                    <div class="lula-mega-grid">
                      ...
                      <a href="{prefix}services/strategy.html" class="lula-mega-item"><span class="lula-mega-icon">🧠</span><div><div class="lula-mega-item-title">Product Strategy</div><div class="lula-mega-item-desc">Discovery, competitive auditing &amp; feature roadmapping.</div></div></a>
...
# 4. DEFINE THE UNIFIED NAVBAR JS
SHARED_NAV_JS = """
...
# 5. BUILD PERFECT PROJECTS.HTML (WORKS)
def build_works_page():
    ...
    # Replace head style in header
    header_clean = re.sub(r'<style id="lula-navbar-v\d+">.*?</style>', '', header, flags=re.DOTALL)
    header_clean = header_clean.replace('</head>', f'{SHARED_NAV_CSS}\n</head>')

    # Replace <nav ...> ... </nav> in header
    header_clean = re.sub(r'<nav.*?</nav>', get_unified_nav_html(prefix=""), header_clean, flags=re.DOTALL)
    ...
    footer_clean = footer_clean.replace('</body>', f'{SHARED_NAV_JS}\n</body>')

    full_projects_html = header_clean + "".join(all_81) + "</div></div>" + footer_clean

    with open("projects.html", "w", encoding="utf-8") as f:
        f.write(full_projects_html)
    print("Generated clean projects.html (works) with 81 cards.")

# 6. BUILD PERFECT INDEX.HTML (HOME)
def build_index_page():
    ...
    html = html.replace('</head>', f'{SHARED_NAV_CSS}\n</head>')

    # Replace navbar markup
    html = re.sub(r'<nav.*?</nav>', get_unified_nav_html(prefix=""), html, flags=re.DOTALL)
    ...
    html = html.replace('</body>', f'{SHARED_NAV_JS}\n</body>')

    with open("index.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("Generated clean index.html (home).")

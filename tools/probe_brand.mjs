...
const errs = [];
vc.on('jsdomError', e => errs.push('jsdomError: ' + String(e.message||e).slice(0,300)));
...
const dom = new JSDOM(html, {
  ...
  beforeParse(window) {
    ...
    window.addEventListener('error', e => errs.push('window error: ' + (e.message||'error') + ' @' + (e.filename||'') + ':' + (e.lineno||'')));
    window.scrollTo = () => {};
  },
});
...
console.log('main h2s:', [...doc.querySelectorAll('main h2')].map(h=>h.textContent.trim()).join(' | '));
console.log('errors:');

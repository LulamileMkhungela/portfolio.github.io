...
// and https://username.github.io/repo/ (project site, e.g. /portfolio.github.io/)
...
(function () {
  try {
    ...
    var siteSections = {
      ...
      'assets': 1, 'lulamile': 1, 'projects': 1
    };
    ...
    // Also allow override via meta tag <meta name="gh-repo" content="...">
    try {
    ...
    // Strip trailing .html or /index or index.html from path without reload so SPA router routes cleanly
    if (window.history && window.history.replaceState) {
      var p = window.location.pathname;
      if (/\.html$/i.test(p) || /\/index$/i.test(p)) {
        var clean = p.replace(/\/index\.html$/i, '/').replace(/\.html$/i, '').replace(/\/index$/i, '');
        if (!clean) clean = '/';
        if (clean !== p) {

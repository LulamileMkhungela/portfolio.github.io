document.addEventListener('DOMContentLoaded', function () {
	...
	function optimizeStaticSite() {
		document.querySelectorAll('a[href]').forEach(function (link) {
			const href = link.getAttribute('href');

			if (!href) return;

			if (href.includes('../..contact/index.html')) {
				link.setAttribute('href', href.replace('../..contact/index.html', '../../contact/index.html'));
			}

			if (isInternalNavigableLink(link)) {
				// Let GitHub Pages handle same-origin page navigation instead of AJAX transitions.
		...
		document.querySelectorAll('.site-main img:not([loading])').forEach(function (img) {
			if (img.closest('.site-header, .site-branding, .project-hero, .project-featured-image, .elementor-widget-projectmedia')) {
				return;
			}
